...
namespace Mulaolao.Class
{
    public class Encrypt
    {
        ...
        public static string GetDataTable ( )
        {
            ...
            strSql.Append( "select net_address from master..sysprocesses where net_address IN ('3417EBF1B1FC','3417EBF1B1FB')" );

            DataTable da= SqlHelper.ExecuteDataTable( strSql.ToString( ) );
            if ( da != null && da.Rows.Count > 0 )
                return encryPt( "LanDE" ,da.Rows[0]["net_address"].ToString( ) );
            else
                return null;
        }
    }
